...
while ( my $line = <GFF> ) {
    ...
    #CDS
    elsif ( ( $type eq 'mRNA' || $type eq 'transcript' ) and ( $. > 1 ) ) {
        ...
        my $cds_coord = $array[0] . " " . $array[3] . " " . $array[4] . " " . $array[6];
        $CDS{$array[3]}=$cds_coord;
        $CDNA{$array[3]}=$cds_coord;
    }
    elsif ($type eq 'UTR' ) {
        my $utr_coord = $array[0] . " " . $array[3] . " " . $array[4] . " " . $array[6];
        $CDNA{$array[3]}=$utr_coord;
    }
    elsif ($type eq 'exon' ) {
        my $exon_coord = $array[0] . " " . $array[3] . " " . $array[4] . " " . $array[6];
...
if(%EXON){
    my $mergedEXON_seq;
        ...
        my $coord = $EXON{$key};
        my @exon_coord = split( " ", $coord);
        my $exon_seq = $db->seq($exon_coord[0],$exon_coord[1], $exon_coord[2], $exon_coord[3]);
        $mergedEXON_seq .= $exon_seq;
    }
    my $output_exon = Bio::Seq->new(
        -seq        => $mergedEXON_seq,
