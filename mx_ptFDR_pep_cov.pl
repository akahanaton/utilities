...
my %opts;
my $usage = qq{
	Usage: $0 <database.file> <maxquant.modificationSpecificPeptides.txt> <proteomicstool.combined.fdr> <sample.code>
};
...
my @sampleCodeIndex;
for(my $i=6; $i<=$#arr; $i++ ){
    if($arr[$i] =~ /Experiment.*$sample_code/){
        push(@sampleCodeIndex, $i);
    }
}

#--------------------------------------------------
# print Dumper(@sampleCodeIndex);
#-------------------------------------------------- 
