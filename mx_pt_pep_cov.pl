...
my %opts;
my $usage = qq{
	Usage: $0 <database.file> <maxquant.modificationSpecificPeptides.txt> <proteomicstool.peptides> <sample.code>
};

getopts('', \%opts);
die($usage) if (@ARGV != 4);
my ($fas, $mx_file, $pt_file, $sample_code) = @ARGV;
...
open(MX, $mx_file) || die("** Fail to open $mx_file.\n");
my $line = <MX>;
my @arr = split('\t', $line);
my @sampleCodeIndex;
for(my $i=6; $i<=$#arr; $i++ ){
    if($arr[$i] =~ /$sample_code/){
        push(@sampleCodeIndex, $i);
    }
}
while( $line = <MX>)
{
    my @arr = split('\t', $line);
    my $evidenceNum = 0;
    foreach my $codeNum (@sampleCodeIndex){
        if ($arr[$codeNum] ne "")
        {
            $evidenceNum++;
        }
    }
    next if $evidenceNum == 0;
    my $curPep = $arr[0];
    $curPep =~ s/I/L/g;
    my @proteinIDs = split(';', $arr[5]);
    my $replaceStr = "#" x length($curPep);
    foreach my $id (@proteinIDs){
        if(defined($refSeq{$id})){
            my $beginPos  =  index($refSeq{$id}, $curPep);
            if($beginPos >= 0){
                $replaceSeq{$id} =~ s/\Q$curPep\E/$replaceStr/g;
            }
            else 
            {
                my $sucess = undef;
                for(my $i=0; $i<= length $curPep; $i++) {
                    my $tmpPep = $curPep;
                    substr($tmpPep, $i, 1, "X");
                    ...
                    if($beginPos >= 0){
                        ...
                        $sucess = 1;
                        #--------------------------------------------------
                        # print STDERR "good mx\t", $curPep,"\t", $tmpPep, "\t", $id, "\t",  $line;
                        #-------------------------------------------------- 
                        last;
                    }
                }
                if(!defined($sucess))
                {
                    print STDERR "mx\t", $curPep,"\t", $id, "\t",  $line;
                }
            }
        }
    }
...
while( $line = <PT>)
{
    ...
    if($arr[2] =~ / ! /){
        ...
        foreach my $curPep (@tmpArr){
            ...
            # print "here1\t",$curPep,"\n";
            #-------------------------------------------------- 
            $sucess = &replace_seq($curPep,$arr[12]);
        }
    } else{
        my $beginIndex = index($arr[2],'.');
        my $endIndex = rindex($arr[2],'.');
        $curPep =  substr $arr[2], $beginIndex+1, $endIndex-$beginIndex-1;
        #--------------------------------------------------
        # print "here2\t",$curPep,"\n";
        #-------------------------------------------------- 
        $sucess = &replace_seq($curPep,$arr[12]);
