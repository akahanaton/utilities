...
def PlotSbjct(out_dir, format, sbjct_id, all_query_seq, all_sbjct_seq, all_blast_result,query_cov_cutoff, sbjct_cov_cutoff, save_tag):
	...
	GapBetweenQuery = 5
	HighCov2Sbjct_Y = SbjctY - GapBetweenQuery
	YticksPos=[SbjctY]
	YticksStr=['BAC']
	cur_query_y = 0 
	sbjct_seq=[0] * sbjct_len
	...
	text(1.02, 0.5, 'Blast Alignment',horizontalalignment='left',	verticalalignment='center',	rotation='vertical',transform=gca().transAxes,clip_on=False)
	#--------------------------------------------------
	# ylabel('Blast Alignment')
	#-------------------------------------------------- 
	title(sbjct_id)
	if save_tag:
