...
=head1 Command-line Option
  ...
  --id <str>       		  specify a seq_id for substructed sequence
  --reverse               reverse the sequence 
  --complement            complement the sequence
  --revall 				  reverse all the sequences in a fasta input
  ...
  --bed                   bed file 
  --idx                   adding a numeric index to the repeat name
  --fam                   output the repeat sequence family by family
...
=head1 Usage Exmples
  ...
  fastaDeal.pl -sub 100-200 -rev -com -id ooo chr1.fa
...
my ($Attribute,$Cuts,$Cutf,$File_in_dir_num,$Pattern,$Unpattern,$Sample, $Bed, $Idx, $Fam);
my ($Substruct,$Reverse,$Complement,$Revall,$Reform,$Outdir,$Id);
my ($Verbose,$Help);
GetOptions(
...
sub cut_fasta{
	...
	while (<IN>) {
		...
		if($num_loop % $seq_num == 0){
			print STDERR "generating:  $out_dir/$mark.fa\n" if( $Verbose && $dir_rank == 1);
			open ( OUT,">$out_dir/$mark.fa" ) if($dir_rank == 1);

			print STDERR "generating:  $out_dir/$submark/$mark.fa\n" if($Verbose && $dir_rank == 2);
			open ( OUT,">$out_dir/$submark/$mark.fa" ) if($dir_rank == 2);
...
sub get_bed{
    ...
    my $gff_name = `basename $gff_file`; ## only file name of $gff_file
    chomp $gff_name;

    my %seqs;
    my $seqin=Bio::SeqIO->new(-file=>$in_file,-format=>"fasta");
    while(my $seq=$seqin->next_seq()){
	$seqs{$seq->id()}=$seq;
    }


    open(G, $gff_file) || die("** Fail to open $gff_file.\n");
    my %seq_info;
    while(my $line = <G>)
    {
	  chomp $line;
	  my @arr = split(/\s+/, $line);
	  if(defined($seq_info{$arr[3]})){
		push @{$seq_info{$arr[3]}{chr}},$arr[0];
		push @{$seq_info{$arr[3]}{begin}},$arr[1];
		push @{$seq_info{$arr[3]}{end}},$arr[2];
	  }else{
		$seq_info{$arr[3]}{chr} = ();
		$seq_info{$arr[3]}{begin} = ();
		$seq_info{$arr[3]}{end} = ();
		push @{$seq_info{$arr[3]}{chr}},$arr[0];
		push @{$seq_info{$arr[3]}{begin}},$arr[1];
		push @{$seq_info{$arr[3]}{end}},$arr[2];
    ...
    mkpath("$Outdir") if defined($Outdir);

    if($Fam){
	  foreach my $seq (keys %seq_info){
		my $out_file = (defined $Outdir) ? "$Outdir/$seq.fa" : "./$seq.fa";
		my @begins = @{$seq_info{$seq}{begin}};
		my @ends = @{$seq_info{$seq}{end}};
		my @chrs = @{$seq_info{$seq}{chr}};
		my $seqOut = Bio::SeqIO->new(-file => ">$out_file", -format => 'fasta');
		for (my $i = 0; $i<=$#begins; $i++){
		    my $out_seq_name = (defined $Idx) ? "$seq\_$i" : "$seq";
		    my $out_seq = Bio::Seq->new(
			  -seq => $seqs{$chrs[$i]}->subseq($begins[$i], $ends[$i]),
			  -display_id => $out_seq_name,
			  -alphabet => "dna" );
	  ...
	  my $out_file = (defined $Outdir) ? "$Outdir/$gff_name.fa" : "./$gff_name.fa";
	  my $seqOut = Bio::SeqIO->new(-file => ">$out_file", -format => 'fasta');
	  foreach my $seq (keys %seq_info){
		my @begins = @{$seq_info{$seq}{begin}};
		my @ends = @{$seq_info{$seq}{end}};
